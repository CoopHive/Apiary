"""Unit tests for the Client class and related functionality.

This module contains unit tests for the Client class and other related components
such as Deal, Event, Job, Match, Result, SmartContract, Solver, and Tx.
"""
...
class TestClient(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.client._create_transaction = lambda value: Tx(
            sender=self.client.get_public_key(), value=value
        )
    ...
    def test_pay_compute_node(self):
        ...
        self.client.smart_contract.balances = {
        ...
        self.assertEqual(
            self.client.smart_contract.balances.get("client_address_123", 0),
            float(2000 - tx_value + 300),
        )
        self.assertEqual(
            self.client.smart_contract.balances.get("resource_provider_address_123", 0),
            tx_value,
        )
    ...
    def test_update_finished_deals(self):
        ...
        self.assertNotIn(
            "deal1", self.client.current_deals
        )  # check if 'deal1' is removed
        self.assertIn(
            "deal2", self.client.current_deals
        )  # check if 'deal2' is still present
        self.assertEqual(
            len(self.client.deals_finished_in_current_step), 0
